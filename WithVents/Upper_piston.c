...
DEFINE_CG_MOTION(piston,dt,vel,omega,time,dtime)
{
    ...
    dv = (dtime * force / 500.0) + (acc * dtime);
    v_prev += dv;

 
    real expected_displacement = total_displacement + (v_prev * dtime);

    if (expected_displacement < -0.09)
    {
        v_prev = 0.0;
        total_displacement = -0.09;
    }
    else
    {
    ...
    /* Write the wall velocity to a file every 50th time step */
    if (time_step_counter == 50)
    {
        FILE *fp;
        fp = fopen("wall_velocity.txt", "a"); /* Open file in append mode */
        if (fp != NULL)
        {
            fprintf(fp, "Time: %f, Wall Velocity: %f\n", time, v_prev);
            fclose(fp);
        }
        time_step_counter = 0; /* Reset the counter */
    }
}
